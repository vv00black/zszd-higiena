...
// NAPRAWA (Aug 24): user zgłosił "jeżeli posiadam plik z konfiguracją,
// powinienem móc ominąć całą procedurę" — wcześniej ten przycisk akceptował
// TYLKO format "paczki startowej" (od administratora), a pełna kopia
// zapasowa (z "Eksportuj kopię teraz") dawała mylący błąd "to nie jest
// prawidłowy plik konfiguracyjny", zmuszając do: załóż konto → zaloguj się
// → wejdź w Ustawienia → znajdź INNY przycisk → wskaż plik jeszcze raz.
// Teraz ten sam przycisk rozpoznaje OBA formaty. Dla pełnej kopii zapasowej
// (brak kont w pliku, bo konta celowo nigdy nie wchodzą do backupu) —
// zapamiętujemy plik i łączymy jego wgranie z zakładaniem konta poniżej,
// w jednym kroku zamiast dwóch osobnych wizyt.
...
document.getElementById('importStarterFile') && document.getElementById('importStarterFile').addEventListener('change', async (e) => {
  ...
  const pendingHint = document.getElementById('pendingBackupHint');
  ...
  try {
    ...
    errEl.textContent = 'Błąd: plik nie jest poprawnym JSON-em.';
    errEl.style.display = 'block';
    e.target.value = '';
    return;
  }

  // Format 1: paczka startowa od administratora (dostęp bez hasła, gotowa rola)
  ...
  // Format 2: pełna kopia zapasowa (np. z innego urządzenia/telefonu) —
  // NIE zawiera kont (celowo, ze względów bezpieczeństwa), więc samo
  // wczytanie tutaj nie wystarczy — trzeba jeszcze konta na tym urządzeniu.
  // Zamiast każdorazowo zmuszać do osobnej wizyty w Ustawieniach: zapamiętaj
  // plik i wgraj go automatycznie zaraz po założeniu konta poniżej.
  if (pkg && pkg.stores && typeof pkg.stores === 'object') {
    pendingBackupImport = pkg;
    pendingHint.textContent = '✅ Rozpoznano pełną kopię zapasową (' + Object.keys(pkg.stores).length + ' modułów). Załóż login i hasło do tego urządzenia poniżej — dane wgrają się automatycznie zaraz po tym.';
    pendingHint.style.display = 'block';
    document.getElementById('firstRunCard').scrollIntoView({ behavior: 'smooth', block: 'start' });
    e.target.value = '';
    return;
  }

  errEl.textContent = 'Błąd: nierozpoznany format pliku (ani paczka startowa, ani kopia zapasowa).';
  errEl.style.display = 'block';
  e.target.value = '';
});
...
document.getElementById('firstRunSubmitBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    delete currentUser.totpSecret;
    delete currentUser.totpBackupCodes;
    // Jeśli user wcześniej wgrał pełną kopię zapasową (patrz importStarterFile
    // wyżej) — wgraj ją teraz automatycznie, w tym samym kroku co zakładanie
    // konta, zamiast zmuszać do osobnej wizyty w Ustawieniach.
    if (pendingBackupImport) {
      await DB.importAllData(pendingBackupImport);
      showToast('Konto założone i kopia zapasowa wgrana');
      pendingBackupImport = null;
    }
...
function applyBuildVersionLabel(label) {
  ...
  // NAPRAWA (Aug 24): etykieta na dole ekranu była zbyt łatwa do przeoczenia,
  // przez co user długo nie zauważał, że patrzy na zamrożoną, starą wersję
  // (mylące z pozoru odświeżenie strony, gdy w rzeczywistości otwarta była
  // zupełnie inna, porzucona instancja aplikacji). Ta sama wersja pokazuje
  // się teraz też widocznie na górze, tuż przy logo.
  const badge = document.getElementById('buildVersionBadge');
  if (badge) badge.textContent = `ADMIN ${label}`;
