...
// ===== Home screen / moduły =====
const MODULES = ['moduleNadziewarki', 'moduleSatelity', 'moduleObecnosc', 'moduleUstawienia', 'moduleCentrala', 'moduleMagazyn', 'moduleSzkolenia', 'moduleHarmCodzienny', 'moduleHarmCykliczny'];
...
function openModule(moduleName) {
  ...
  if (moduleName === 'nadziewarki') {
    ...
    if (typeof initDesktopFolderPanel === 'function') initDesktopFolderPanel();
    if (typeof initStarterFolderPanel === 'function') initStarterFolderPanel();
    ...
    if (typeof renderDesktopPackagesList === 'function') renderDesktopPackagesList();
    if (typeof loadDriveFolderSetting === 'function') loadDriveFolderSetting();
...
document.getElementById('checkUpdateBtn').addEventListener('click', async () => {
  // W wersji desktopowej aktualizacja polega na podmianie pliku .exe — nie ma Service Workera
  if (window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop) {
    showToast('Wersja desktopowa — aktualizujesz ją podmieniając plik .exe na nowszy');
    return;
  }
...
async function sendToCentrala(auto = false) {
  ...
  if (auto) {
    // WERSJA DESKTOPOWA: zapisz paczkę wprost do folderu centrali — bez żadnego klikania.
    if (window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop) {
      const folder = await window.ZSZD_DESKTOP.getSyncFolder();
      if (folder) {
        const res = await window.ZSZD_DESKTOP.savePackage(filename, JSON.stringify(pkg, null, 2), true);
        await DB.setSetting('lastSentAt', new Date().toISOString());
        if (res.ok) {
          // Zapisane w folderze — nic nie zostaje do zrobienia
          await DB.setSetting('pendingUploadFile', '');
          await updateLastSentHint();
          await renderPendingUploadBanner();
          showToast('Paczka zapisana w folderze centrali');
          return;
        }
        // Nie udało się zapisać — pokaż baner z informacją
        await DB.setSetting('pendingUploadFile', filename);
        await updateLastSentHint();
        await renderPendingUploadBanner();
        showToast('Nie udało się zapisać do folderu: ' + res.error);
        return;
      }
      // Brak wybranego folderu — poproś o wskazanie
      showToast('Wskaż folder na paczki danych w Ustawieniach');
    }
    // TRYB AUTOMATYCZNY (przeglądarka): cicho zapisz plik i powiadom
...
// ===== WERSJA DESKTOPOWA: folder na paczki danych =====
async function initDesktopFolderPanel() {
  const card = document.getElementById('desktopFolderCard');
  if (!card) return;
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) {
  ...
  const folder = await window.ZSZD_DESKTOP.getSyncFolder();
  const pathEl = document.getElementById('desktopFolderPath');
  pathEl.textContent = folder ? ('Folder: ' + folder) : 'Nie wskazano jeszcze folderu — paczki nie zapiszą się automatycznie.';
}

document.getElementById('chooseSyncFolderBtn') && document.getElementById('chooseSyncFolderBtn').addEventListener('click', async () => {
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) return;
  const folder = await window.ZSZD_DESKTOP.chooseSyncFolder();
  if (folder) showToast('Folder ustawiony');
  await initDesktopFolderPanel();
});

document.getElementById('openSyncFolderBtn') && document.getElementById('openSyncFolderBtn').addEventListener('click', async () => {
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) return;
  const res = await window.ZSZD_DESKTOP.openSyncFolder();
  if (!res.ok) showToast(res.error || 'Nie udało się otworzyć folderu');
});

async function initStarterFolderPanel() {
  const card = document.getElementById('starterFolderCard');
  if (!card) return;
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) {
    card.style.display = 'none';
    return;
  ...
  card.style.display = 'block';
  const folder = await window.ZSZD_DESKTOP.getStarterFolder();
  const pathEl = document.getElementById('starterFolderPath');
  pathEl.textContent = folder ? ('Folder: ' + folder) : 'Nie wskazano jeszcze folderu — przy generowaniu paczki okno "Zapisz jako" otworzy się bez podpowiedzianej lokalizacji.';
}

document.getElementById('chooseStarterFolderBtn') && document.getElementById('chooseStarterFolderBtn').addEventListener('click', async () => {
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) return;
  const folder = await window.ZSZD_DESKTOP.chooseStarterFolder();
  if (folder) showToast('Folder ustawiony');
  await initStarterFolderPanel();
});

document.getElementById('openStarterFolderBtn') && document.getElementById('openStarterFolderBtn').addEventListener('click', async () => {
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) return;
  const res = await window.ZSZD_DESKTOP.openStarterFolder();
  if (!res.ok) showToast(res.error || 'Nie udało się otworzyć folderu');
});
...
// ===== WERSJA DESKTOPOWA: paczki prosto z folderu (Centrala) =====
async function renderDesktopPackagesList() {
  const card = document.getElementById('desktopPackagesCard');
  if (!card) return;
  if (!(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) {
    card.style.display = 'none';
    return;
  ...
  if (!folder) {
    card.style.display = 'block';
    document.getElementById('desktopPackagesList').innerHTML =
      '<div class="hint">Najpierw wskaż folder na paczki w module Ustawienia.</div>';
    return;
  }
  card.style.display = 'block';
  const files = await window.ZSZD_DESKTOP.listPackages();
  const listEl = document.getElementById('desktopPackagesList');
  if (!files.length) {
    listEl.innerHTML = '<div class="hint">Brak plików .json w folderze.</div>';
    return;
  }
  listEl.innerHTML = files.map(f => {
    const kb = Math.round(f.size / 1024);
    const when = formatDateTime(f.mtime);
    const icon = f.name.includes('nadzor') ? '🧭' : (f.name.includes('bryg') ? '👷' : '📄');
    return `
      <div class="storage-row" style="margin-bottom:6px;">
        <span>${icon} <strong>${escapeHtml(f.name)}</strong><br>
          <span style="font-size:12px;color:var(--text-dim);">${kb} KB · ${when}</span>
        </span>
        <button class="btn secondary" data-load-package="${escapeHtml(f.name)}">Wczytaj</button>
      </div>`;
  }).join('');

  listEl.querySelectorAll('[data-load-package]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const name = btn.dataset.loadPackage;
      const res = await window.ZSZD_DESKTOP.readPackage(name);
      if (!res.ok) { showToast('Błąd odczytu: ' + res.error); return; }
      try {
        const pkg = JSON.parse(res.content);
        if (!pkg || pkg.kind !== 'centrala-submission') {
          alert('To nie jest paczka danych dla centrali.');
          return;
        }
        await DB.saveCentralSubmission(pkg);
        showToast('Paczka wczytana: ' + (pkg.brygadzista ? pkg.brygadzista.name : name));
        await renderCentrala();
      } catch (e) {
        alert('Błąd wczytywania paczki: ' + e.message);
      }
    });
  });
}

document.getElementById('refreshPackagesBtn') && document.getElementById('refreshPackagesBtn').addEventListener('click', renderDesktopPackagesList);
...
let lastStarterPackagePath = null;
...
document.getElementById('generateStarterBtn') && document.getElementById('generateStarterBtn').addEventListener('click', async () => {
  ...
  const resultBox = document.getElementById('starterSaveResult');
  ...
  try {
    ...
    if (role === 'obszar') {
      ...
      roleSubfolder = 'Koordynatorzy';
    } else {
      fname = `${prefix}-admin-telefon.json`;
      roleSubfolder = 'Admin';
    }
    const content = JSON.stringify(starter, null, 2);
    const instrukcja = cel === 'naprawa'
      ? 'Prześlij ten plik osobie na tym stanowisku. W aplikacji, na ekranie głównym, w sekcji "🆘 Pomoc — coś nie działa?" niech wybierze "📥 Wczytaj dane naprawcze".'
      : 'Prześlij ten plik osobie na tym stanowisku. Przy pierwszym uruchomieniu aplikacji zobaczy ekran z prośbą o wczytanie tego pliku.';

    if (window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop) {
      // Wersja desktopowa: okno "Zapisz jako" z podpowiedzianą lokalizacją
      // (folder startowy + podfolder wg roli, jeśli folder jest ustawiony).
      const res = await window.ZSZD_DESKTOP.saveStarterPackage(fname, content, roleSubfolder);
      if (res.canceled) return;
      if (!res.ok) { alert('Błąd zapisu paczki: ' + (res.error || '')); return; }
      lastStarterPackagePath = res.path;
      document.getElementById('starterSaveResultText').textContent = 'Zapisano: ' + res.path + '\n\n' + instrukcja;
      document.getElementById('starterSaveResult').style.display = 'block';
      showToast('Paczka zapisana');
      return;
    }

    // Wersja przeglądarkowa/PWA: zwykłe pobranie pliku
...
document.getElementById('starterShowInFolderBtn') && document.getElementById('starterShowInFolderBtn').addEventListener('click', async () => {
  if (!lastStarterPackagePath || !(window.ZSZD_DESKTOP && window.ZSZD_DESKTOP.isDesktop)) return;
  await window.ZSZD_DESKTOP.showItemInFolder(lastStarterPackagePath);
});
