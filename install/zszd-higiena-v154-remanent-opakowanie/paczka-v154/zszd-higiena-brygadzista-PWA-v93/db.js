...
function openDB() {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      // Przełączanie kont na jednym urządzeniu (v10) — pełne migawki innych
      // profili (tożsamość + wszystkie dane), żeby kilka osób mogło korzystać
      // z tej samej instalacji na jednym telefonie, każda ze swoim kontem.
      if (!db.objectStoreNames.contains('savedProfiles')) {
        db.createObjectStore('savedProfiles', { keyPath: 'id' });
      }
      // Kolejka wysyłek do centrali przez internet (v11) — gdy urządzenie jest
...
const BACKUP_EXCLUDE_STORES = ['autoBackups', 'users', 'savedProfiles', 'firebaseQueue']; // kopie zapasowe, konta użytkowników, zapisane profile innych kont i kolejka wysyłek są ściśle per-urządzenie, nigdy nie wchodzą do backupu/importu/wysyłki do centrali
...
DB.buildSubmissionPackage = async (username) => {
...
// Buduje "paczkę startową" dla aplikacji brygadzisty — konfigurację którą admin
// przygotowuje PRZED przekazaniem aplikacji brygadziście. Zawiera:
// - tożsamość brygadzisty (który to wpis z listy),
// - jego obszary + wszystkich brygadzistów tych obszarów (do 3 sekcji check-listy),
// - jego pracowników (przypisanych do brygadzistów jego obszarów),
// - wspólne dane modułów (katalogi usterek, maszyny, urządzenia, magazyn) do pracy,
// - nadane uprawnienia do modułów.
DB.buildBrygadzistaStarter = async (brygadzistaEntryId, allowedModules = []) => {
  const brygadzisci = await DB.getBrygadzisciList();
  const obszary = await DB.getObszary();
  const employees = await DB.getAll('employees');

  const me = brygadzisci.find(b => b.id === brygadzistaEntryId);
  ...
  // Pracownicy przypisani do tych brygadzistów + wpisy pracownicze brygadzistów
  ...
  // Wspólne dane modułów (żeby brygadzista mógł pracować w Nadziewarkach/Satelitach/Magazynie)
  const base = await DB.exportAllData();
  // Podmieniamy dane obecności na wyłącznie te dotyczące brygadzisty
...
const ALL_MODULE_KEYS = ['nadziewarki', 'satelity', 'obecnosc', 'magazyn', 'centrala', 'ustawienia'];
...
// ===== APLIKACJA BRYGADZISTY: tożsamość i import paczki startowej =====
// Tożsamość brygadzisty (kim jest ta instancja aplikacji) — zapisana lokalnie po imporcie paczki.
...
// Importuje paczkę startową od admina: wgrywa dane i zapisuje tożsamość brygadzisty.
DB.importBrygadzistaStarter = async (pkg) => {
  if (!pkg || pkg.kind !== 'brygadzista-starter' || !pkg.brygadzistaIdentity) {
    throw new Error('To nie jest prawidłowa paczka startowa dla brygadzisty.');
  }
  // Jeśli na tym urządzeniu było już jakieś konto — zachowaj je jako zapisany
  // profil, zanim je nadpiszemy nowym (żeby dało się do niego wrócić przez
  // "Przełącz konto", zamiast bezpowrotnie je stracić).
  await DB.snapshotCurrentProfile();
  await DB.wipeAllData(); // czyste miejsce na nowy profil — dane poprzedniego nie mieszają się z nowym
  await DB.importAllData({ stores: pkg.stores });
  // Zapisz tożsamość + uprawnienia
...
// ===== PRZEŁĄCZANIE KONT NA JEDNYM URZĄDZENIU =====
// Kilka osób może korzystać z tej samej instalacji na jednym telefonie —
// każda ze swoim kontem. Zamiast osobnych instalacji (które i tak dzieliłyby
// tę samą bazę danych na urządzeniu — PWA nie rozróżnia "kopii" tej samej
// apki), trzymamy pełne migawki innych profili w osobnym, nietykalnym
// miejscu (savedProfiles) i podmieniamy aktywne dane w locie.
DB.snapshotCurrentProfile = async () => {
  const identity = await DB.getBrygadzistaIdentity();
  if (!identity || !identity.id) return null; // nic jeszcze nie ma na tym urządzeniu — nie ma czego zapisywać
  const label = `${identity.imie || ''} ${identity.nazwisko || ''}`.trim()
    || (identity.role === 'admin' ? 'Admin (telefon)' : identity.role === 'koordynator' ? 'Koordynator' : 'Brygadzista');
  const snapshot = await DB.exportAllData(); // obejmuje WSZYSTKIE dane robocze + samą tożsamość (settings)
  const profile = { id: identity.id, label, role: identity.role || 'brygadzista', savedAt: Date.now(), snapshot };
  await DB.put('savedProfiles', profile);
  return profile;
};

DB.getSavedProfiles = () => DB.getAll('savedProfiles');

DB.switchToProfile = async (profileId) => {
  await DB.snapshotCurrentProfile(); // zachowaj to, co jest teraz aktywne, zanim to opuścisz
  const profile = await DB.get('savedProfiles', profileId);
  if (!profile) throw new Error('Nie znaleziono zapisanego konta.');
  await DB.wipeAllData();
  await DB.importAllData(profile.snapshot); // przywraca dane robocze ORAZ tożsamość (obie są w "stores")
};

DB.deleteSavedProfile = (id) => DB.delete('savedProfiles', id);
