...
function openMagProductModal(productId) {
  ...
  if (productId) {
    ...
    // Aktualny stan — TO SAMO computeStock() co w liście "Stany magazynowe",
    // pokazane teraz też tutaj, w oknie produktu, żeby nie trzeba było
    // zamykać okna i szukać go w liście z tyłu.
    const aktStanWrap = document.getElementById('magProductAktualnyStanWrap');
    const aktStanVal = document.getElementById('magProductAktualnyStanValue');
    if (aktStanWrap && aktStanVal) {
      const s = computeStock(p);
      let packInfo = '';
      if (p.wielkoscOpak && p.wielkoscOpak > 0) {
        const packCount = s.stanBiezacy / p.wielkoscOpak;
        const packDisplay = Number.isInteger(packCount) ? packCount : packCount.toFixed(2);
        packInfo = ` (${packDisplay} szt. × ${p.wielkoscOpak} ${escapeHtml(p.jm || '')})`;
      }
      aktStanVal.innerHTML = `${s.stanBiezacy} ${escapeHtml(p.jm || '')}${packInfo} <span class="badge ${s.status === 'OK' ? 'ok' : 'bad'}" style="margin-left:6px;">${s.status}</span>`;
      aktStanWrap.style.display = 'block';
    }
    ...
    // Nowy produkt jeszcze nie ma żadnej historii — nie ma czego liczyć,
    // więc pole "Aktualny stan" pozostaje ukryte (patrz gałąź wyżej).
    const aktStanWrapNew = document.getElementById('magProductAktualnyStanWrap');
    if (aktStanWrapNew) aktStanWrapNew.style.display = 'none';
    ...
    return `
      <div class="checklist-row" style="flex-wrap:wrap;align-items:center;max-width:none;${tloStyl}">
        <div style="flex:1;min-width:200px;">
          <div style="font-weight:600;">${escapeHtml(p.nazwa)}${p.indeks ? ` <span class="hint">(${escapeHtml(p.indeks)})</span>` : ''}${p.wielkoscOpak ? ` <span class="hint">— ${p.wielkoscOpak} ${escapeHtml(p.jm || '')}/opak.</span>` : ''}</div>
