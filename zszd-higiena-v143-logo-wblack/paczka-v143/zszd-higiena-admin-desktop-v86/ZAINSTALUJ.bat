...
echo.
echo ==========================================
echo   ZSZD Higiena ADMIN - instalacja
echo ==========================================
echo.
...
set "NAZWA_EXE=zszd-higiena-admin-v%WERSJA%.exe"
echo Wersja aplikacji: %WERSJA%
echo.

REM ---- Znajdz plik .exe ----
set "ZRODLO="
for %%F in ("%~dp0dist\ZSZD-Higiena-ADMIN-portable-*.exe") do set "ZRODLO=%%~fF"
if not defined ZRODLO (
  for %%F in ("%~dp0ZSZD-Higiena-ADMIN-portable-*.exe") do set "ZRODLO=%%~fF"
)
...
if not defined ZRODLO (
  ...
  echo.
  echo Szukalem:
  echo   %~dp0dist\ZSZD-Higiena-ADMIN-portable-*.exe
  echo   %~dp0ZSZD-Higiena-ADMIN-portable-*.exe
  echo.
...
echo.
echo   [1] %USERPROFILE%\ZSZD Higiena          (zalecane - Twoj folder domowy)
echo   [2] %LOCALAPPDATA%\ZSZD Higiena         (ukryty folder aplikacji)
...
if "%WYBOR%"=="1" set "CEL=%USERPROFILE%\ZSZD Higiena"
if "%WYBOR%"=="2" set "CEL=%LOCALAPPDATA%\ZSZD Higiena"
...
REM nazwa ZSZD-Higiena-ADMIN.exe sprzed wprowadzenia numerow w nazwie) -
REM zeby folder nie zasmiecal sie kolejnymi wersjami przy kazdej aktualizacji.
echo Kopiuje aplikacje...
if exist "%CEL%\zszd-higiena-admin-v*.exe" del /q "%CEL%\zszd-higiena-admin-v*.exe" 2>nul
if exist "%CEL%\ZSZD-Higiena-ADMIN.exe" del /q "%CEL%\ZSZD-Higiena-ADMIN.exe" 2>nul
...
set "SKROT_LNK=%PULPIT%\ZSZD Higiena ADMIN.lnk"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$s = (New-Object -ComObject WScript.Shell).CreateShortcut($env:SKROT_LNK); ^
   ...
   $s.Description = 'ZSZD Higiena ADMIN'; ^
   $s.Save()" 2>nul

if exist "%PULPIT%\ZSZD Higiena ADMIN.lnk" (
...
if exist "%MENUSTART%" (
  set "SKROT_LNK=%MENUSTART%\ZSZD Higiena ADMIN.lnk"
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$s = (New-Object -ComObject WScript.Shell).CreateShortcut($env:SKROT_LNK); ^
     $s.TargetPath = $env:SKROT_EXE; ^
     $s.WorkingDirectory = $env:SKROT_DIR; ^
     $s.Save()" 2>nul
  if exist "%MENUSTART%\ZSZD Higiena ADMIN.lnk" echo   Skrot w Menu Start utworzony.
)
...
echo %APPDATA%\zszd-higiena-admin
