# ZSZD Higiena BRYGADZISTA — wersja na Windows
...
**Aktualizacja:** po zbudowaniu nowej wersji uruchom `ZAINSTALUJ.bat` ponownie i wskaż ten sam folder. Plik zostanie podmieniony, skróty zostają, dane też — leżą w `%AppData%\zszd-higiena-brygadzista`, niezależnie od tego, gdzie jest sam program.
...
Rozpakuj folder `zszd-higiena-brygadzista-desktop` w wygodne miejsce, na przykład:

```
C:\ZSZD\zszd-higiena-brygadzista-desktop
```
...
Powinno otworzyć się okno aplikacji ZSZD Higiena BRYGADZISTA. Zamknij je, żeby wrócić do konsoli.
...
```
dist\ZSZD-Higiena-BRYGADZISTA-portable-1.6.3.exe
```
...
```
C:\Users\TwojaNazwa\AppData\Roaming\zszd-higiena-brygadzista
```
...
- **Ta sama, ujednolicona aplikacja jest też dostępna jako wersja przeglądarkowa (PWA), dołączona w tej samej paczce
